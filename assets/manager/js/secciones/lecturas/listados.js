...
//  }
  ...
  function resetLoteModal() {
    ...
    $("#lote_validation_title").html("");
    $("#lote_validation_detail").html("");
    $("#lote_action_errores, #lote_action_sin_index, #lote_action_completo").hide().attr("href", "#");
    loteProcesando = false;
    loteValidado = false;
  }

  function actualizarKpisProceso(total, procesados, erroresApi) {
    var porcentaje = total > 0 ? Math.round(((procesados + erroresApi) / total) * 100) : 0;
  ...
  function renderValidacionLote(result) {
    var $box = $("#lote_validation_result");
    var clase = "ok";

    if (result.estado === "incompleto") {
      clase = "danger";
    } else if (result.estado === "observaciones") {
      clase = "warning";
    } else if (result.estado === "vacio") {
      clase = "danger";
    }
  ...
  function validarLoteProcesado(closeIfOk) {
    ...
    $("#lote_validation_result").removeClass("ok warning danger").addClass("warning").show();
    $("#lote_validation_title").html("Validando lote...");
    $("#lote_validation_detail").html("Recalculando errores, sin index y estado del lote.");
    ...
    $.ajax({
      ...
      url: $("body").data("base_url") + "Lotes/validar_lote",
      success: function (result) {
        loteValidado = true;
        loteProcesando = false;
      ...
      error: function (xhr) {
        loteProcesando = false;
        $("#lote_validation_result").removeClass("ok warning").addClass("danger").show();
        $("#lote_validation_title").html("No se pudo validar el lote");
        $("#lote_validation_detail").html("El servidor respondio " + xhr.status + ". Revisar el lote manualmente.");
      },
    });
  }

  window.prepararModalProcesamientoLote = function () {
  ...
  $("body").off("click", "button#cerrar_modal").on("click", "button#cerrar_modal", function (e) {
    e.preventDefault();
  ...
  $("body").off("click", "button#enviar_archivos").on("click", "button#enviar_archivos", function (e) {
    e.preventDefault();
    $("button#enviar_archivos").attr("disabled", "disabled");
    $(".progress").show();
    $("#lote_validation_result").hide().removeClass("ok warning danger");
...
$(function () {
  var myFileUploadDropZone = new Dropzone(".dropzone", {
    ...
    acceptedFiles: ".pdf",
    addRemoveLinks: true,
    dictDefaultMessage: "Arrastra facturas PDF o haz click para seleccionarlas",
    ...
    queuecomplete: function (file, xhr, formData) {
      ...
      if (window.prepararModalProcesamientoLote) {
        window.prepararModalProcesamientoLote();
      }
