...
class Lotes extends backend_controller
{
	...
	public function getDato($file, $id_proveedor)
	{
		...
		$query = "SELECT id, dato_api FROM _datos_api WHERE nombre_archivo = ?";
		$valor = $file;
...
if ($resultado) {
	...
	public function leerApi()
	{
		...
		$this->getDato($file, $proveedor->id);
	...
	public function viewBatch($id = null)
	{
		...
		if ($this->input->is_ajax_request()) {
			...
			foreach ($LotesData as $r) {
				...
				$accionesDelete = '<span data-id_lote="' . $r->id . '" data-code="' . $r->code_lote . '"class="borrar_lote acciones" data-consolidado="' . $r->consolidado . '"><a title="Borrar lote" href="#"  class=""><i class=" text-danger icon-trash " title="Borrar "></i> </a> </span>';
