...
class Dependencias extends backend_controller
{
	function __construct()
	{
		...
		if (!$this->ion_auth->logged_in()) {
			...
			$this->table  = '_dependencias_canon';
			$this->data['tabla'] = '_dependencias_canon';
	...
	public function get_dependencias($demo = false)
	{
		// demo por que no tenemos asignadas las dependencias en las indexaciones
		if($demo){
			$query = $this->db->select('id,CONCAT(dependencia," -  ", direccion )as dependencia')->get('_dependencias_canon');
	...
	public function index($id = NULL)
	{
		...
		if ($id && $id != NULL) {
			...
			$editData = $this->Electromecanica_model->get_data('_dependencias_canon', $id);
		...
		if ($_SERVER['REQUEST_METHOD'] === "POST") {
			...
			if ($this->form_validation->run() != FALSE) {
				...
				if (isset($_REQUEST['id_dependencia']) && $_REQUEST['id_dependencia'] != NULL) {
					...
					redirect(base_url('Electromecanica/Dependencias'));
				}

				$this->Electromecanica_model->grabar_datos($this->table, $datos);
				redirect(base_url('Electromecanica/Dependencias'));
			}
		}
	...
	public function list_dt()
	{
		...
		if ($this->input->is_ajax_request()) {
			...
			foreach ($memData as $r) {
				...
				$accionesEdit = '<span data-id_="' . $r->id . '" class="editar_ acciones" ><a title="Editar" href="/Electromecanica/' . ucfirst($this->router->fetch_class()) . '/editar/' . $r->id_dependencia . '"  class=""><i class=" text-warningr  icon-pencil4 " title="Editar "></i> </a> </span>';
				...
				$acciones = '<ul class="icons-list">
					<li class="text-primary-600"><a  href="/Electromecanica/' . ucfirst($this->router->fetch_class()) . '/editar/' . $r->id_dependencia . '"><i class="icon-pencil7"></i></a></li>
	...
	public function delete()
	{
		try {
			...
			$this->db->delete('_dependencias_canon');
