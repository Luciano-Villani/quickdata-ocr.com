	...
	public function copy($id = 0)
{
    if ($this->input->is_ajax_request()) {
        ...
        // Verificar duplicados de cuentas
        ...
        $duplicados = [];
        foreach ($registro_factura as $reg) {
            if (in_array($reg->nro_cuenta, $cuentas)) {
                $duplicados[] = $reg->nro_cuenta;
            } else {
                $cuentas[] = $reg->nro_cuenta;
            }
        }
        ...
        // Verificar si la factura está duplicada en otro lote
        $nro_factura = $datoleido->nro_factura;

        // Revisa el contenido de $datoleido para asegurarte de que tiene la propiedad id
var_dump($datoleido);
die();

// Verifica si la propiedad id está definida antes de usarla
if (isset($datoleido->id)) {
    $registro_factura_existente = $this->Manager_model->get_alldata('_datos_api', 'nro_factura = "' . $nro_factura . '" AND id != ' . $datoleido->id);
} else {
    // Manejo del error
    $registro_factura_existente = null;
    log_message('error', 'La propiedad "id" no está definida en el objeto $datoleido.');
}
        ...
        $result = array(
            ...
            'hayDuplicadoFactura' => $hayDuplicadoFactura,
    ...
    if ($id == 0 && $_SERVER['REQUEST_METHOD'] === "POST") {
        ...
        if ($this->form_validation->run() != FALSE) {
            ...
            $timestamp = strtotime($_REQUEST['vencimiento_del_pago']);
            $_REQUEST['vencimiento_del_pago'] = ($timestamp === false) ? 'error de lectura' : date('Y-m-d', $timestamp);

            $timestamp = strtotime($_REQUEST['fecha_emision']);
            $_REQUEST['fecha_emision'] = ($timestamp === false) ? 'error de lectura' : date('Y-m-d', $timestamp);
    ...
    $importe_total = (count($datoTotalesMultiple) > 0) ? $datoTotalesMultiple[0]->total_importe : 0;
