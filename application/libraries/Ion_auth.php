...
class Ion_auth
{
		...
		if ($user) {
			...
			if ($code) {
				...
				if (!$this->config->item('use_ci_email', 'ion_auth')) {
					...
					$data['user'] = $user;
					$data['subject'] = $this->config->item('site_title', 'ion_auth') . ' - ' . $this->lang->line('email_forgotten_password_subject');


					if ($this->_enviar_email($data)) {
						...
						echo 'sa'.$this->email->print_debugger();die();
					}
					...
					if ($this->email->send()) {
						...
						echo 'sa' . $this->email->print_debugger();
						die();
					}
				}
			}
	...
	function _enviar_email($data)
	{
		...
		$this->load->library('phpmailer_lib');
			...
			$mail->setFrom('no-reply@quickdata.com.ar', 'quickdata');
			// $mail->addReplyTo('mvl@quickdata.com.ar', 'quickdata');
	...
	{
		$this->ion_auth_model->trigger_events('is_ventas');

		$ventas_group = $this->config->item('ventas_group', 'ion_auth');

		return $this->ion_auth_model->in_group($ventas_group, $id);
	}

	public function is_marketing($id = FALSE)
	{
		$this->ion_auth_model->trigger_events('is_marketing');

		$marketing_group = $this->config->item('marketing_group', 'ion_auth');

		return $this->ion_auth_model->in_group($marketing_group, $id);
	}
